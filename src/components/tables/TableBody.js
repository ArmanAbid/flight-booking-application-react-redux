...
import { deleteFlight } from "../../redux/flight/action";

export default function TableBody(){
  const flightState = useSelector(state => state.flights)
  const dispatch = useDispatch()

  const flightDeleteHandler = (id) =>{
    dispatch(deleteFlight({
      id: id
    }))
  }

    return flightState.map(flight =>(
          ...
          <tr className="lws-bookedTable text-black">
            ...
            <td className="px-6 py-4 text-center">
              <div className="flex justify-center gap-4">
                <button className="lws-remove" onClick={() => flightDeleteHandler(flight.id)}>
